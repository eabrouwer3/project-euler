...
**Core data flow:**
1. Problem list and descriptions are fetched from `projecteuler.net/minimal=*` and cached in-memory (the list for 1 hour, a description for the life of the process). Descriptions link images, data files and other problems relative to projecteuler.net's document root, so `resolveLinks` absolutises them — without it every problem image renders broken. `localiseDownloadHints` rewrites the site's "right click and 'Save Link/Target As...'" in the same pass: the file it tells the reader to download is already in the working directory the run executes in, so following the instruction only puts a copy on a laptop the solution never runs on
...
4. Code execution: `POST /api/run` runs one command with a 60s timeout — Project Euler's own one-minute rule — in a **Railway Sandbox**: a VM booted from a toolchain checkpoint, held per user and reused across their runs. Opening a problem warms one in the background; it expires ~5 minutes after the last run. Supports Python, TypeScript (Bun), Ruby, Clojure, Rust, C++, and AArch64 assembly (GNU as). Assembly is the one language that does not run on the host's own architecture: the sandbox is x86-64, so a solution is assembled by the `aarch64-linux-gnu` cross binutils and run under `qemu-aarch64-static`, which costs a few times native speed against the one-minute deadline
   ...
   **A checkpoint belongs to one region.** `SANDBOX_REGION` pins the sandboxes to where these services run, and everything snapshotted along the way inherits it: a template built in Railway's default region produces a checkpoint that cannot boot in ours, and Railway refuses it with `Checkpoint "…" lives in us-west2 and cannot boot in us-east4-eqdc4a`. Forwarding the region to the template build needs the Railway SDK at 3.10 or later, and the checkpoint's name carries the region because `Sandbox.checkpoints()` does not report one — without it a snapshot from a previous region is indistinguishable from a usable one until every run fails on it.

   Pinning the region only governs a *new* build, though: Railway content-addresses a built template by its instructions, so a build that already exists in another region is handed back whatever region asks for it, and code execution stays down until that snapshot is deleted. `planRecovery` is what unsticks it without anyone logging into Railway — delete the snapshot Railway named and build again, and if the rebuild lands in the same foreign region anyway, run sandboxes there rather than nowhere, saying so in the logs. Both branches are bounded: a toolchain build costs minutes, so a Railway that keeps answering with a new unbootable snapshot must not be able to spend builds indefinitely.
